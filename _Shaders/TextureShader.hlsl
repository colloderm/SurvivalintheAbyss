...
float4 PS(PixelInput input) : SV_Target
{
    ...
    {
        // 흑백 처리
        float3 gray = dot(color.rbg, float3(0.299f, 0.587f, 0.114f)); // 게임업계에서 자주 쓰는 흑백코드 float3(0.299f, 0.587f, 0.114f)
    ...
    else if (_selection == 4)
    {
        ...
        // 프로그래머가 지정한 블러의 세기만큼 반복하며 (1 == 블러 안함)
    ...
    if(_alpha == true)
    {
        color = color - (color * _alphaCount) + (_alphaColor * _alphaCount); // _alpha == 내가 선택한 색값
    };
